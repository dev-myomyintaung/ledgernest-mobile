...
const STEPS = [
    {
        targetId: 'scan-camera',
        icon: 'camera.fill' as const,
        title: 'Snap it live',
        description:
            'Point your camera at a receipt and take a photo. The clearer the shot, the better the scan.',
    },
    {
        targetId: 'scan-library',
        icon: 'photo.fill' as const,
        title: 'Already have a photo?',
        description:
            'Upload a receipt image from your photo library — works great for e-receipts or screenshots.',
    },
];

type Rect = { x: number; y: number; width: number; height: number };

export function ScanTour() {
    ...
    const completeScanTour = useOnboardingStore((s) => s.completeScanTour);

    const [step, setStep] = useState(0);
    const [spotlight, setSpotlight] = useState<Rect | null>(null);
    const fadeAnim = useRef(new Animated.Value(0)).current;

    const visible = firstBudgetPromptDone && !scanTourDone;
    ...
    // Card goes below the spotlight when target is in the upper half of the screen,
    // above it when in the lower half.
    const spotlightMidY = sl ? sl.y + sl.h / 2 : SCREEN_H / 2;
    const targetInUpperHalf = spotlightMidY < SCREEN_H / 2;
    const cardPosition = sl
        ? targetInUpperHalf
            ? { top: sl.y + sl.h + 24 }
            : { bottom: SCREEN_H - sl.y + 24 }
        : { top: SCREEN_H * 0.35 };
